...
internal class ToolbarSceneSelection : BaseToolbarElement
{
	...
	{
		selectedSceneIndex = EditorGUILayout.Popup(selectedSceneIndex, scenesPopupDisplay.Select(e => e.popupDisplay).ToArray(), GUILayout.Width(200));
	...
	void RefreshScenesList()
	{
		...
		{
			if (scenesPath[i].Contains("Assets/[Game]/Scenes"))
			{
	...
	void InitScenesData()
	{
		...
		sceneGuids = AssetDatabase.FindAssets("t:scene", new string[] { "Assets", "Packages" });
