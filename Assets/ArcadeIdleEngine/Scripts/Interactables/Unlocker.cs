	...
	public class Unlocker : MonoBehaviour
	{
		...
		void SpendMoney(int x)
		{
			...
			if (_inventory.TryRemove(_requiredResource, out Item item))
			{
				...
				TweenHelper.Jump(item.transform, transform.position, _jumpHeight, 1, _jumpDuration, item.ReleaseToPool);
				amount++;
				while (amount < decreasingAmountDelta)
				{
					if (_inventory.TryRemove(_requiredResource, out Item it))
					{
						...
						TweenHelper.Jump(it.transform, transform.position, _jumpHeight, 1, _jumpDuration, it.ReleaseToPool);
						amount++;	
					}
				}
			}
			else
			{
				...
				if (_spawnCount >= VISUAL_FEEDBACK_SPAWN_RATE_MAX + 1 - _visualFeedbackSpawnRate)
				{
					...
					TweenHelper.Jump(trans, transform.position, _jumpHeight, 1, _jumpDuration, poolInstance.ReleaseToPool);
					_spawnCount = 0;
				}
			}
